...
slot_lr1=5e-4
slot_lr2=1e-4
intra_consistency_reg_coeff=0.1
...
s2p_mode=attn+soft
s2p_temp=$1
for slot_logit_similar_reg_coeff in 0.01; do
slot_logit_similar_reg_temp=0.001
slot_logit_similar_reg_slot_temp=30    # 0.1, 1

LOGNAME=22-slot-icr${intra_consistency_reg_coeff}_m${intra_consistency_reg_mode}-sor${slot_ortho_reg_coeff}_t${slot_ortho_reg_temp}-s2p_m${s2p_mode}_t${s2p_temp}-cheating-slsrc${slot_logit_similar_reg_coeff}_old_t${slot_logit_similar_reg_temp}_${slot_logit_similar_reg_slot_temp}-lr${lr}-p100-l8-k10-nt5
